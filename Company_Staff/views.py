...
def addemployeloan(request):
    if request.method=='POST':
        ...
        if log_details.user_type == "Company":
            ...
            data=EmployeeLoan(Employee=employee,LoanAmount=Loan_Amound,Loandate=loandate,Expiry_date=experydate,note=Note,file=file,company=dash_details,status='Active',
        ...
        if log_details.user_type == 'Staff':
            ...
            data=EmployeeLoan(Employee=employee,LoanAmount=Loan_Amound,Loandate=loandate,Expiry_date=experydate,note=Note,file=file,company=dash_details.company,status='Active',
...
def delete_repayment(request,id):
    ...
    if log_details.user_type == 'Company':
        ...
        return redirect('employeeloan_details',loan_id)
    if log_details.user_type == 'Staff':
        dash_details = StaffDetails.objects.get(login_details=log_details)
        repay = EmployeeLoanRepayment.objects.get(id=id)
        loan_id = repay.emp.id
    ...
    if request.method=='POST':
        ...
        if log_details.user_type == 'Company':
            ...
            for entry in gt_entries:
                ...
                entry.save()
                bal = entry.balance
            return redirect('employeeloan_details',repay.emp.id)
        if log_details.user_type == 'Staff':
            dash_details = StaffDetails.objects.get(login_details=log_details)
            repay = EmployeeLoanRepayment.objects.get(id=id)
            repay.principal_amount = request.POST.get('principal')
